...
<link rel="stylesheet" type="text/css"
	href="${pageContext.request.contextPath}/merchant/css/sales_style.css">
	...
	<div class="container">
		<div class="content-container">
			<h1 class="sales-header">
				매출 내역 <span class="sales-subtitle">현재까지 판매한 매출내역을 확인할 수 있습니다.</span>
			</h1>
			<div class="flex-container">
				<div class="flex-item">
					...
					<p class="info-text">
						• 고객이 사용한 포인트는 OZ의 집에서 모두 부담하므로, 정산에 포함되지 않습니다.<br> • 정산에
				...
				<form name="f" action="${pageContext.request.contextPath}/merchants/${merchantLoginMember.mer_num}/sales"
				 -->
				<form name="f"
					action="${pageContext.request.contextPath}/merchants/sales"
					class="flex-container" method="post">
					<!-- 로그인 연결하면 merNum 수정하기 -->
					<div class="flex-row">
						<c:if test="${map.mode eq 'day'}">
							...
							<div class="flex-cell input-cell">
								<input type="text" id="startDate" name="startDate"
									...
									id="endDate" name="endDate" value="${map.endDate}"> <input
									type="hidden" name="mode" value="day"> <input
						...
						<c:if test="${map.mode eq 'month'}">
							...
							<div class="flex-cell input-cell">
								...
								<input type="hidden" name="mode" value="month"> <input
									type="submit" value="검색">
							</div>
						</c:if>
						<c:if test="${map.mode eq 'year'}">
							...
							<div class="flex-cell input-cell">
								<select name="sYear" class="custom-select">${options.syOptions}</select>
								~ <select name="eYear" class="custom-select">${options.eyOptions}</select>
								<input type="hidden" name="mode" value="year"> <input
									type="submit" value="검색">
							</div>
						</c:if>
					</div>
					...
					<c:if test="${ map.mode eq 'day' }">
						...
						<div class="scroll flex-container content-table full-width-table">
							<div class="flex-row header-row">
								<div class="flex-cell">
									주문번호
									<div class="sub-header">ID</div>
								</div>
								<div class="flex-cell">
									주문일
									<div class="sub-header">취소일</div>
								</div>
							...
							<c:forEach var="dto" items="${salesList}">
								<c:set var="plus"
									value="${dto.product_price + dto.product_assembly_cost}" />
								<c:set var="minus"
									value="${dto.order_dis_coupon + dto.order_dis_discount}" />
								...
								<div class="flex-row">
									...
									<div class="flex-cell">
										<fmt:formatNumber value="${dto.product_price}" type="number"
											pattern="###,###" />
									</div>
									<div class="flex-cell">
										<fmt:formatNumber value="${dto.product_assembly_cost}"
											type="number" pattern="###,###" />
									</div>
									<div class="flex-cell">
										<fmt:formatNumber value="${dto.order_dis_coupon}"
											type="number" pattern="###,###" />
									</div>
									<div class="flex-cell">
										<fmt:formatNumber value="${dto.order_dis_discount}"
											type="number" pattern="###,###" />
									</div>
									<div class="flex-cell">
										<fmt:formatNumber value="${charge}" type="number"
											pattern="###,###" />
									</div>
									<div class="flex-cell">
										<fmt:formatNumber value="${calculate}" type="number"
											pattern="###,###" />
									</div>
									<div class="flex-cell">
										<c:choose>
									...
									<c:set var="allproduct_price"
										value="${allproduct_price + dto.product_price}" />
									<c:set var="allProduct_assembly_cost"
										value="${allProduct_assembly_cost + dto.product_assembly_cost}" />
									<c:set var="allOrder_dis_coupon"
										value="${allOrder_dis_coupon + dto.order_dis_coupon}" />
									<c:set var="allOrder_dis_discount"
										value="${allOrder_dis_discount + dto.order_dis_discount}" />
				...
				<div class="scroll flex-container content-table full-width-table">
					...
					<c:forEach var="dto" items="${salesList}">
						...
						<div class="flex-row">
							...
							<div class="flex-cell">
								<fmt:formatNumber value="${dto.order_dis_coupon}" type="number"
									pattern="###,###" />
							</div>
							<div class="flex-cell">
								<fmt:formatNumber value="${dto.order_dis_discount}"
					...
					<div class="flex-content">
						<c:choose>
							...
							<c:otherwise>
								<div class="flex-row">
									<div class="flex-cell">
										<fmt:formatNumber value="${allproduct_price}" type="number"
											pattern="###,###원" />
									</div>
									<div class="flex-cell">
										<fmt:formatNumber value="${allProduct_assembly_cost}"
											type="number" pattern="###,###원" />
									</div>
									<div class="flex-cell">
										<fmt:formatNumber
											value="${allproduct_price + allProduct_assembly_cost}"
											type="number" pattern="###,###원" />
									</div>
									<div class="flex-cell">
										<fmt:formatNumber value="${allOrder_dis_coupon}" type="number"
											pattern="###,###원" />
									</div>
									<div class="flex-cell">
										<fmt:formatNumber value="${allOrder_dis_discount}"
											type="number" pattern="###,###원" />
									</div>
									<div class="flex-cell">
										<fmt:formatNumber value="${allCharge}" type="number"
											pattern="###,###원" />
									</div>
									<div class="flex-cell">
										<fmt:formatNumber value="${allCalcul}" type="number"
											pattern="###,###원" />
									</div>
								</div>
							</c:otherwise>
