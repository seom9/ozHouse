...
<div class="container">
	<div class="content-container">
		...
		<div class="flex-container">
			...
			<form name="f1"
				action="${pageContext.request.contextPath}/merchants/proQAs/search"
					...
					<div class="flex-cell input-cell">
						<select name="product_QA_state" id="product_QA_state">
							...
							<option value="answer_wait"
								${param.product_QA_state == 'answer_wait' ? 'selected' : ''}>답변대기</option>
							<option value="answer_complete"
								${param.product_QA_state == 'answer_complete' ? 'selected' : ''}>답변완료</option>
						</select>
					</div>
				</div>
				<div class="flex-row">
				...
				<div class="flex-row">
					...
					<div class="flex-cell input-cell">
						<select name="search" id="search">
							...
							<option value="product_inquiry_type"
								${param.search == 'product_inquiry_type' ? 'selected' : ''}>문의유형</option>
							<option value="product_qa_content"
								${param.search == 'product_qa_content' ? 'selected' : ''}>내용</option>
							<option value="product_name"
								${param.search == 'product_name' ? 'selected' : ''}>제품명</option>
		...
		<div class="scroll flex-container content-table">
			...
			<c:forEach var="dto" items="${productQAList}">
				<div class="flex-row">
					...
					<div class="flex-cell">
						<a
							href="product_qa_content.do?product_QA_num=${dto.product_QA_num }">
							<c:choose>
					...
					<div class="flex-cell">
						<c:if test="${dto.product_QA_state == 'f'}">
							<input type="button" class="update-button"
								...
								onclick="window.location='product_reqa.do?product_QA_num=${dto.product_QA_num}'">
							<!-- 
						...
						<c:if test="${dto.product_QA_state == 't'}">
							<input type="button" class="update-button"
								...
								onclick="window.location='product_reqa_update.do?product_reQA_num=${dto.product_QA_num}'">
