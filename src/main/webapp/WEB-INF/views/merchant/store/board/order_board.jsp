...
<div class="container">
	<div class="content-container">
		...
		<div class="flex-container">
			<form name="f1"
				action="${pageContext.request.contextPath}/merchants/orderQAs/search"
				class="flex-container">
				<!-- 
			<form name="f1" action="${pageContext.request.contextPath}/merchants/${merchantLoginMember.merNum}/orderQAs/search"
				class="flex-container">
				 -->
				<input type="hidden" name="merNum"
					value="${merchantLoginMember.merNum }"> <input
					type="hidden" name="order_num" value="${order_num}">
				<div class="flex-row">
					...
					<div class="flex-cell input-cell">
						<select name="order_QA_state" id="order_QA_state">
							...
							<option value="answer_wait"
								${param.order_QA_state == 'answer_wait' ? 'selected' : ''}>답변대기</option>
							<option value="answer_complete"
								${param.order_QA_state == 'answer_complete' ? 'selected' : ''}>답변완료</option>
						</select>
					</div>
				</div>
				<div class="flex-row">
					...
					<div class="flex-cell input-cell">
						<select name="search" id="search">
							...
							<option value="order_num"
								${param.search == 'order_num' ? 'selected' : ''}>주문번호</option>
							<option value="order_inquiry_type"
								${param.search == 'order_inquiry_type' ? 'selected' : ''}>문의유형</option>
							<option value="order_qa_content"
								${param.search == 'order_qa_content' ? 'selected' : ''}>내용</option>
							<option value="member_id"
								${param.search == 'member_id' ? 'selected' : ''}>작성자</option>
					...
					<div class="flex-cell">
						<c:if test="${dto.order_QA_state == 'f'}">
							<input type="button" class="update-button"
								style="background-color: #50E5B4; color: white;" value="답변"
								onclick="window.location='order_reqa.do?order_QA_num=${dto.order_QA_num}'">
							<!--         
						...
						<c:if test="${dto.order_QA_state == 't'}">
							<input type="button" class="update-button"
								style="background-color: #50E5B4; color: white;" value="수정"
								onclick="window.location='order_reqa_update.do?order_reQA_num=${dto.order_QA_num}'">
							<!-- 
